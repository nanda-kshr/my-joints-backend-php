...
if ($method === 'POST') {
    ...
    $db = getDB();
    try {
        $stmt = $db->prepare("INSERT INTO treatments (uid, treatment, name, dose, route, frequency, frequency_text, time_period) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    $db = getDB();
    try {
        $stmt = $db->prepare("SELECT * FROM treatments WHERE uid = ? ORDER BY createdAt DESC LIMIT 20");
